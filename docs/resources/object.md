...
```terraform
# Configure the provider with PATCH method for Midpoint
provider "restapi" {
  alias                = "midpoint"
  uri                  = "http://midpoint-server:8080/midpoint/api"
  update_method        = "PATCH"  # Enable Midpoint ObjectModificationType
...
resource "restapi_object" "midpoint_user" {
  provider = restapi.midpoint
  path     = "/users"
  ...
  # When data changes, only the changes will be sent to Midpoint
  # using PATCH with the ObjectModificationType format
}
